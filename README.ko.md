...
> **Android APK (Android 12+)** — Kotlin 보조 앱으로 원격 ompgui 서버에 연결하고, 최신 세션의 읽기 전용 오프라인 스냅샷을 이용할 수 있습니다. [ompgui Remote v0.7.4 다운로드](https://github.com/dbc-hbin/ompgui/releases/download/v0.7.4/ompgui-remote-v0.7.4.apk) · [릴리스 노트](https://github.com/dbc-hbin/ompgui/releases/tag/v0.7.4)
...
- **메시지 대기열**: 웹과 현재 Android 클라이언트는 서버 소유의 메모리 내 대기열을 공유합니다. Android에서는 **Session controls → Queue**를 직접 열며, 입력 중 Execution/Queue 막대가 자동으로 나타나지 않습니다. 전송 전 항목은 텍스트와 이미지를 입력창으로 회수하거나 삭제할 수 있고, 후속 메시지를 안전한 전송 시점을 따르는 조종 메시지로 승격할 수 있습니다. 전송 중 항목은 편집할 수 없습니다. 스냅샷에는 이미지 메타데이터만 포함되고 원본 바이트는 비공개로 보관되며 개수·바이트·메모리 제한이 적용됩니다. 15 MiB를 초과하는 Relay 회수는 변경 전에 거부하고 웹 사용을 안내합니다. 서버 프로세스 재시작 시 대기열은 보존되지 않으며, 기존 배포 클라이언트의 프롬프트 진입점은 호환됩니다.
- **세션 검색**: 메타데이터 또는 색인 기반 본문 모드를 선택하고, 시작일 포함·종료일 제외 필터와 읽기 전용 일치 문맥을 사용할 수 있습니다. 비공개 Node/SQLite 파생 색인은 표시되는 대화 텍스트를 대상으로 하며 자격 증명, 숨겨진 추론, 불투명 페이로드는 색인하지 않습니다. 검색은 기록을 수정하지 않습니다.
- **공급자 확인**: 구성 검증은 네트워크 테스트가 아닙니다. 실제 연결 테스트는 비용 발생 가능성에 대한 명시적 승인 후 고정 프롬프트와 최대 32 출력 토큰으로 공급자 요청을 한 번 이하 전송합니다. 직접 입력한 후보 자격 증명의 OpenAI Chat Completions·OpenAI Responses·Anthropic Messages API 형식만 지원하며 저장된 OAuth 자격 증명은 사용하지 않습니다.
