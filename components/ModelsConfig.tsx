...
function NativeRegistryDetail({ models, connectedProviders, onChanged }: { models: RuntimeModelEntry[]; connectedProviders: ConnectedProvider[]; onChanged: () => Promise<void> }) {
  ...
  if (!settings) return <div style={{ color: "var(--text-muted)", fontSize: "var(--text-md)" }}>Loading native OMP registry settings...</div>;
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    <div>
      ...
      <p style={{ margin: "4px 0 0", color: "var(--text-muted)", fontSize: "var(--text-md)", lineHeight: 1.5 }}>These settings affect the models OMP itself can resolve. They are saved in <code>~/.omp/agent/config.yml</code>, unlike the Composer picker.</p>
    </div>
    <section style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-card)", overflow: "hidden" }}>
      <label style={{ display: "flex", alignItems: "center", gap: "var(--space-4)", padding: "10px 12px", background: "var(--bg-panel)", color: "var(--text)", fontSize: "var(--text-md)", fontWeight: 600 }}><input type="checkbox" checked={allowListEnabled} disabled={saving || isReadOnly} onChange={(event) => void save({ ...settings, enabledModels: event.target.checked ? allModelKeys : [] })} /> Restrict OMP to selected models</label>
      <p style={{ margin: 0, padding: "8px 12px", color: "var(--text-muted)", fontSize: "var(--text-sm)", lineHeight: 1.45 }}>{allowListEnabled ? "Unchecked models are unavailable to every OMP session." : "All currently available OMP models are allowed."}</p>
      {allowListEnabled && <div style={{ maxHeight: 260, overflowY: "auto", borderTop: "1px solid var(--border)" }}>{models.map((model) => {
        const key = `${model.provider}/${model.id}`;
        return <label key={key} style={{ display: "flex", alignItems: "center", gap: "var(--space-4)", padding: "6px 12px", color: "var(--text-muted)", fontSize: "var(--text-md)" }}><input type="checkbox" checked={enabledModels.has(key)} disabled={saving || isReadOnly} onChange={(event) => { const next = new Set(enabledModels); if (event.target.checked) next.add(key); else next.delete(key); void save({ ...settings, enabledModels: [...next] }); }} /><code>{key}</code></label>;
      })}</div>}
    </section>
    <section style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-card)", overflow: "hidden" }}>
      <div style={{ padding: "10px 12px", background: "var(--bg-panel)", color: "var(--text)", fontSize: "var(--text-md)", fontWeight: 600 }}>Disabled Providers</div>
      <p style={{ margin: 0, padding: "8px 12px", color: "var(--text-muted)", fontSize: "var(--text-sm)", lineHeight: 1.45 }}>Disabling a provider removes it from OMP&apos;s model registry, even if it has credentials.</p>
      <div style={{ borderTop: "1px solid var(--border)" }}>{providers.map((provider) => <label key={provider} style={{ display: "flex", alignItems: "center", gap: "var(--space-4)", padding: "7px 12px", color: "var(--text-muted)", fontSize: "var(--text-md)" }}><input type="checkbox" checked={disabledProviders.has(provider)} disabled={saving || isReadOnly} onChange={(event) => { const next = new Set(disabledProviders); if (event.target.checked) next.add(provider); else next.delete(provider); void save({ ...settings, disabledProviders: [...next] }); }} /><ProviderIcon id={provider} size={14} /><code>{provider}</code></label>)}</div>
    </section>
    <section style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-card)", overflow: "hidden" }}>
      <div style={{ padding: "10px 12px", background: "var(--bg-panel)", color: "var(--text)", fontSize: "var(--text-md)", fontWeight: 600 }}>Provider Preference</div>
      <p style={{ margin: 0, padding: "8px 12px", color: "var(--text-muted)", fontSize: "var(--text-sm)", lineHeight: 1.45 }}>Sets OMP&apos;s provider order when a model id is ambiguous.</p>
      <div style={{ borderTop: "1px solid var(--border)" }}>{orderedProviders.map((provider, index) => <div key={provider} style={{ display: "flex", alignItems: "center", gap: "var(--space-4)", padding: "7px 12px", color: "var(--text-muted)", fontSize: "var(--text-md)" }}><ProviderIcon id={provider} size={14} /><code style={{ flex: 1 }}>{provider}</code><button type="button" disabled={saving || isReadOnly || index === 0} onClick={() => { const next = [...orderedProviders]; [next[index - 1], next[index]] = [next[index], next[index - 1]]; void save({ ...settings, modelProviderOrder: next }); }} title="Move provider up" aria-label={`Move provider ${provider} up`} className="ui-focus-ring" style={{ width: 24, height: 24, padding: 0, display: "inline-flex", alignItems: "center", justifyContent: "center", border: "none", borderRadius: 4, background: "transparent", color: "var(--text-muted)", cursor: "pointer" }}><ArrowUp size={14} /></button><button type="button" disabled={saving || isReadOnly || index === orderedProviders.length - 1} onClick={() => { const next = [...orderedProviders]; [next[index + 1], next[index]] = [next[index], next[index + 1]]; void save({ ...settings, modelProviderOrder: next }); }} title="Move provider down" aria-label={`Move provider ${provider} down`} className="ui-focus-ring" style={{ width: 24, height: 24, padding: 0, display: "inline-flex", alignItems: "center", justifyContent: "center", border: "none", borderRadius: 4, background: "transparent", color: "var(--text-muted)", cursor: "pointer" }}><ArrowDown size={14} /></button></div>)}</div>
    </section>
    {isReadOnly && <div role="status" style={{ padding: "9px 11px", border: "1px solid var(--border)", borderRadius: "var(--radius-control)", color: "var(--text-muted)", fontSize: "var(--text-md)", lineHeight: 1.45 }}>OMP path-scoped registry entries are configured. Edit <code>config.yml</code> directly to preserve their path rules.</div>}
    {error && <div role="alert" style={{ color: "var(--status-error)", fontSize: "var(--text-md)" }}>{error}</div>}
  </div>;
}
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: "var(--text-sm)", fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 2 }}>{children}</div>;
}
  ...
  return (
    <button
      ...
      style={{
        ...
        cursor: "pointer", fontSize: "var(--text-md)", textAlign: "left", display: "flex", alignItems: "center", gap: "var(--space-4)",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-6)" }}>
      ...
      <div style={{ padding: "14px 16px", border: "1px solid var(--border)", borderRadius: "var(--radius-card)", background: "var(--bg-panel)", display: "flex", flexDirection: "column", gap: 10, boxShadow: "var(--shadow-card)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "var(--space-5)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              ...
              <div style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)", fontFamily: "var(--font-mono)" }}>{hostName}</div>
            </div>
          </div>
        ...
        <div style={{ borderTop: "1px solid var(--border)", paddingTop: 10, marginTop: 4 }}>
          <div style={{ fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>
            Quick Endpoint Presets
          </div>
      ...
      {/* Danger Zone */}
      <section style={{ padding: "14px 16px", border: "1px solid color-mix(in srgb, var(--status-error) 25%, transparent)", borderRadius: "var(--radius-card)", background: "var(--bg-panel)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "var(--space-5)" }}>
        <div>
          <div style={{ fontSize: 12.5, fontWeight: 600, color: "var(--text)" }}>Remove Provider</div>
          <div style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)", marginTop: 2 }}>Delete {name} and remove its models from models.yml configuration.</div>
        </div>
        <button
          type="button"
      ...
      {THINKING_LEVELS.map((level) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ display: "flex", borderRadius: "var(--radius-control)", border: `1px solid ${state === "string" ? "var(--accent)" : "var(--border)"}`, overflow: "hidden", transition: "border-color var(--dur-fast) var(--ease-out-warm)" }}>
              <button
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-6)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
          ...
          <span style={{ fontSize: "var(--text-sm)", color: provider.loggedIn ? "var(--status-success)" : "var(--text-dim)" }}>
      ...
      <div style={{ minHeight: 48 }}>
        ...
        {loginState.phase === "connecting" && (
          <p style={{ margin: 0, fontSize: "var(--text-md)", color: "var(--text-muted)" }}>{t("modelsConfig.openingBrowser")}</p>
        )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
              {loginState.options.map((option) => (
                <button
                  ...
                  style={{ padding: "6px 9px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-control)", color: "var(--text)", cursor: "pointer", fontSize: "var(--text-md)", textAlign: "left" }}
                >
                  {option.label}
                </button>
              ))}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ display: "flex", gap: "var(--space-3)" }}>
              <input
                ...
                style={{ flex: 1, padding: "6px 9px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-control)", color: "var(--text)", fontSize: "var(--text-md)", outline: "none", fontFamily: "var(--font-mono)", boxSizing: "border-box" }}
              />
              <button
                onClick={() => submitCode(loginState.token, inputValue)}
                disabled={!inputValue.trim()}
                style={{ padding: "6px 12px", background: inputValue.trim() ? "var(--accent)" : "var(--bg-panel)", border: "none", borderRadius: "var(--radius-control)", color: inputValue.trim() ? "var(--on-accent)" : "var(--text-dim)", cursor: inputValue.trim() ? "pointer" : "not-allowed", fontSize: "var(--text-md)", fontWeight: 600, flexShrink: 0 }}
              >
        ...
        {loginState.phase === "device_code" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ padding: "8px 10px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-control)", color: "var(--text)", fontSize: 16, fontWeight: 700, fontFamily: "var(--font-mono)", letterSpacing: 0 }}>
              {loginState.userCode}
            </div>
            <p style={{ margin: 0, fontSize: "var(--text-sm)", color: "var(--text-dim)", lineHeight: 1.5 }}>
        ...
        {loginState.phase === "success" && (
          <p style={{ margin: 0, fontSize: "var(--text-md)", color: "var(--status-success)" }}>{t("modelsConfig.connectedSuccessfully")}</p>
        )}
        {loginState.phase === "error" && (
          <p style={{ margin: 0, fontSize: "var(--text-md)", color: "var(--status-error)" }}>{loginState.message}</p>
        )}
      </div>
      ...
      <div style={{ display: "flex", gap: "var(--space-4)" }}>
        {isWorking ? (
          <button
            ...
            style={{ padding: "5px 12px", background: "none", border: "1px solid var(--border)", borderRadius: "var(--radius-control)", color: "var(--text-muted)", cursor: "pointer", fontSize: "var(--text-md)" }}
          >
          ...
          <>
            <button
              onClick={handleLogin}
              style={{ padding: "5px 14px", background: "var(--accent)", border: "none", borderRadius: "var(--radius-control)", color: "var(--on-accent)", cursor: "pointer", fontSize: "var(--text-md)", fontWeight: 600 }}
            >
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-6)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
          ...
          <span style={{ fontSize: "var(--text-sm)", color: provider.configured ? "var(--status-success)" : "var(--text-dim)" }}>
...
function AddProviderPicker({
  ...
  const cardStyle: CSSProperties = {
    display: "flex", flexDirection: "row", alignItems: "center", gap: "var(--space-4)",
    padding: "10px 12px",
    background: "var(--bg-panel)",
    border: "1px solid var(--border)",
    borderRadius: "var(--radius-control)",
    ...
    <Dialog open onOpenChange={(o) => { if (!o) onClose(); }}>
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "4px 14px 14px" }}>
          {totalCount === 0 ? (
            <div style={{ padding: "20px 0", fontSize: "var(--text-md)", color: "var(--text-dim)", textAlign: "center" }}>{t("modelsConfig.noProvidersMatch")}</div>
          ) : (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(240px, 100%), 1fr))", gap: "var(--space-4)" }}>
              {showCustom && (
                <div style={{ gridColumn: "1 / -1", fontSize: "var(--text-xs)", fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.07em" }}>{t("modelsConfig.customSection")}</div>
              )}
              {showCustom && (
                <button
                  type="button"
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: "var(--text-md)", fontWeight: 600, color: "var(--text)", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t("modelsConfig.openaiAnthropicCompatible")}</div>
                    <div style={{ fontSize: "var(--text-xs)", color: "var(--text-dim)", marginTop: 2 }}>{t("modelsConfig.customEndpointFormat")}</div>
                  </div>
              ...
              {availableOAuth.length > 0 && (
                <div style={{ gridColumn: "1 / -1", paddingTop: showCustom ? 6 : 0, fontSize: "var(--text-xs)", fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.07em" }}>{t("modelsConfig.subscriptions")}</div>
              )}
              {availableOAuth.map((p) => (
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: "var(--text-md)", fontWeight: 600, color: "var(--text)", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</div>
              ...
              {availableApiKey.length > 0 && (
                <div style={{ gridColumn: "1 / -1", paddingTop: availableOAuth.length > 0 ? 6 : 0, fontSize: "var(--text-xs)", fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.07em" }}>{t("modelsConfig.apiKey")}</div>
              )}
              {availableApiKey.map((p) => (
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: "var(--text-md)", fontWeight: 600, color: "var(--text)", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.displayName}</div>
                    <div style={{ fontSize: "var(--text-xs)", color: "var(--text-dim)", marginTop: 2 }}>{tn("modelsConfig.modelCount", p.modelCount)}</div>
                  </div>
                  <ProviderIcon id={p.id} size={28} />
                </button>
              ))}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {runtimeModelsLoading ? <div style={{ color: "var(--text-muted)", fontSize: "var(--text-md)" }}>Loading OMP runtime models...</div> : Object.entries(runtimeModelsByProvider).map(([provider, models]) => {
          ...
          return <section key={provider} style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-card)", overflow: "hidden" }}>
            <label style={{ display: "flex", alignItems: "center", gap: "var(--space-4)", padding: "9px 11px", background: "var(--bg-panel)", color: "var(--text)", fontSize: "var(--text-md)", fontWeight: 600 }}>
              ...
              <span>{provider}</span>
              <span style={{ marginLeft: "auto", color: "var(--text-dim)", fontSize: "var(--text-sm)", fontWeight: 400 }}>{models.length} model{models.length === 1 ? "" : "s"}</span>
            </label>
            <div style={{ padding: "4px 0" }}>
              {models.map((model) => (
                <label key={`${model.provider}:${model.id}`} style={{ display: "flex", alignItems: "center", gap: 9, padding: "7px 11px", color: "var(--text-muted)", cursor: "pointer" }}>
                  ...
                  <span style={{ minWidth: 0, flex: 1, fontSize: "var(--text-md)" }}>{model.name || model.id}</span>
                  <code style={{ color: "var(--text-dim)", fontSize: "var(--text-sm)", fontFamily: "var(--font-mono)", whiteSpace: "nowrap" }}>{model.provider}/{model.id}</code>
                </label>
              ))}
            </div>
          </section>;
        })}
        {!runtimeModelsLoading && runtimeModels.length === 0 && <div style={{ color: "var(--text-muted)", fontSize: "var(--text-md)" }}>OMP did not report any configured models.</div>}
        {connectedProviders.filter((provider) => !runtimeModelsByProvider[provider.id]).map((provider) => (
          <section key={provider.id} style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-card)", padding: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: "var(--space-4)", color: "var(--text)", fontSize: "var(--text-md)", fontWeight: 600 }}><ProviderIcon id={provider.id} size={15} />{provider.name}</div>
            <p style={{ margin: "6px 0 0", color: "var(--text-muted)", fontSize: "var(--text-md)", lineHeight: 1.5 }}>{provider.disabled ? "Connected, but disabled in OMP. Enable it to discover its models." : "Connected, but OMP has not reported models for this provider yet."}</p>
            {provider.disabled && <button type="button" onClick={() => void enableConnectedProvider(provider.id).catch((error) => toast.error("Could not enable provider", error instanceof Error ? error.message : String(error)))} style={{ marginTop: 8, padding: "6px 10px", border: "1px solid var(--border)", borderRadius: "var(--radius-control)", background: "transparent", color: "var(--text)", cursor: "pointer", fontSize: "var(--text-md)" }}>Enable in OMP</button>}
          </section>
        ))}
      </div>
    );
      ...
      <ModelsConfigSurface embedded={embedded} isMobile={isMobile} onClose={onClose}>
        ...
        {parseError ? (
          <div style={{ flex: 1, overflowY: "auto", padding: "var(--space-8)", display: "flex", flexDirection: "column", gap: "var(--space-5)" }}>
            ...
            <div style={{ fontSize: "var(--text-md)", color: "var(--text-muted)", lineHeight: 1.6 }}>{t("modelsConfig.parseErrorBody")}</div>
            {parseError.path && (
              <code style={{ fontSize: "var(--text-sm)", color: "var(--text-dim)", fontFamily: "var(--font-mono)", wordBreak: "break-all" }}>{parseError.path}</code>
            )}
            <pre style={{
          ...
          <div style={{
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "8px 6px" }}>
              ...
              {(activeOAuth.length > 0 || activeApiKey.length > 0) && (
                <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", padding: "14px 10px 6px", color: "var(--text-dim)", fontSize: "var(--text-xs)", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase" }}>
              ...
              {activeOAuth.map((p) => {
                ...
                return (
                  <button
                    ...
                    style={{ display: "flex", alignItems: "center", gap: "var(--space-4)", padding: "7px 10px", borderRadius: "var(--radius-control)", cursor: "pointer", width: "100%", border: "none", textAlign: "left", fontFamily: "inherit", background: isSelected ? "var(--bg-selected)" : "none", fontWeight: isSelected ? 600 : 400 }}
                    {...hoverRow(isSelected)}
                  >
                    <ProviderIcon id={p.id} size={16} />
                    <span style={{ fontSize: "var(--text-md)", color: "var(--text)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</span>
              ...
              {activeApiKey.map((p) => {
                ...
                return (
                  ...
                  >
                    <ProviderIcon id={p.id} size={16} />
                    <span style={{ fontSize: "var(--text-md)", color: "var(--text)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.displayName}</span>
              ...
              {providers.length > 0 && (
                <div style={{ padding: "10px 10px 4px", fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
              ...
              {/* Custom providers */}
              {loading ? (
                <div style={{ padding: "10px 8px", fontSize: "var(--text-md)", color: "var(--text-muted)" }}>{t("modelsConfig.loading")}</div>
                ...
                return (
                  <div key={pName} style={{ marginBottom: 6, padding: "4px 6px", borderRadius: "var(--radius-control)", background: isProviderSelected ? "var(--bg-panel)" : "transparent", border: isProviderSelected ? "1px solid var(--border)" : "1px solid transparent" }}>
                    ...
                    >
                      ...
                      <span style={{ fontSize: "var(--text-md)", fontWeight: isProviderSelected ? 600 : 500, color: "var(--text)", fontFamily: "var(--font-mono)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {pName}
                      </span>
                      <span style={{ fontSize: "var(--text-xs)", color: "var(--text-dim)", padding: "1px 5px", borderRadius: 4, background: "var(--bg-subtle)" }}>
                    ...
                    {models.map((m, i) => {
                      ...
                      return (
                        <button
                          ...
                          style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", padding: "5px 8px 5px 28px", borderRadius: "var(--radius-control)", cursor: "pointer", width: "100%", border: "none", textAlign: "left", fontFamily: "inherit", background: isModelSelected ? "var(--bg-selected)" : "none", marginTop: 1 }}
                          {...hoverRow(isModelSelected)}
                        >
                          <span style={{ fontSize: "var(--text-sm)", fontFamily: "var(--font-mono)", color: m.id ? "var(--text-muted)" : "var(--text-dim)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    ...
                    {/* Add model buttons */}
                    <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", padding: "6px 8px 4px 10px", marginTop: 2, flexWrap: "nowrap" }}>
                      <button
                        type="button"
                        onClick={(e) => { e.stopPropagation(); addModel(pName); }}
                        style={{ display: "inline-flex", alignItems: "center", gap: "var(--space-2)", padding: "3px 8px", borderRadius: "var(--radius-control)", cursor: "pointer", color: "var(--text-muted)", border: "1px solid var(--border)", background: "var(--bg-subtle)", fontFamily: "inherit", fontSize: "var(--text-sm)", whiteSpace: "nowrap", flexShrink: 0 }}
                        {...hoverAccent}
                      >
            ...
            <div style={{ borderTop: "1px solid var(--border)", padding: "8px 6px" }}>
              <button onClick={() => setPickerOpen(true)} style={{
                ...
                color: "var(--text-muted)", cursor: "pointer", fontSize: "var(--text-md)",
              }}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 10, padding: "10px 18px", borderTop: "1px solid var(--border)", flexShrink: 0 }}>
          ...
          <button onClick={handleSave} disabled={saving || savedOk || parseError !== null} style={{
            ...
            cursor: (saving || savedOk || parseError) ? "default" : "pointer", fontSize: "var(--text-base)", fontWeight: 600,
