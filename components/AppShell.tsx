...
export function AppShell() {
  ...
  return (
    ...
    {settingsTab && <SettingsConfig activeTab={settingsTab} onAdvisorChange={handleAdvisorChange} toolCallsDefaultCollapsed={toolCallsDefaultCollapsed} onToolCallsDefaultCollapsedChange={handleToolCallsDefaultCollapsedChange} cwd={activeCwd ?? selectedSession?.cwd ?? newSessionCwd} sessionId={selectedSession?.id ?? null} systemPrompt={systemPrompt} systemPromptLoading={systemPromptLoading} onLoadSystemPrompt={handleLoadSystemPrompt} onModelsSaved={() => setModelsRefreshKey((k) => k + 1)} onPluginsReloaded={() => setSessionKey((k) => k + 1)} onOmpSessionsRestarted={() => setOmpVersionRefreshKey((key) => key + 1)} onOmpUpdateAvailabilityChange={setOmpUpdateAvailable} onAppUpdateAvailabilityChange={setAppUpdateAvailable} onSelectTab={setSettingsTab} onClose={() => setSettingsTab(null)} runtimeReady={selectedSession === null || runtimeReady} />}
