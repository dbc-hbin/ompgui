...
test("allows reverse proxy / HTTPS tunnel requests (e.g. Tailscale Funnel) via x-forwarded headers", async () => {
  const { isApiRequestOriginAllowed } = await loadSubject();
  const request = new Request("http://127.0.0.1:30177/api/test", {
    method: "POST",
    headers: {
      host: "hanbin-macbookpro.tail2632d9.ts.net:8443",
      origin: "https://hanbin-macbookpro.tail2632d9.ts.net:8443",
