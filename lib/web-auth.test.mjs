...
const { createWebSession, isValidWebPassword, isValidWebSession, isWebPasswordEnabled, OMPGUI_SESSION_COOKIE } = await jiti.import("./web-auth.ts");
...
test("redirects browser requests to the password screen and blocks unauthenticated APIs", () => {
  ...
  try {
    ...
    const primaryCookieResponse = proxy(new NextRequest("http://localhost:30177/", {
      headers: { cookie: `${OMPGUI_SESSION_COOKIE}=${session}` },
    }));
