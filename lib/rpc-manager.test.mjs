...
test("registered host tools route to listeners; unknown ones are rejected", async () => {
  ...
  assert.match(source, /this\.pendingHostTools\.set\(id, this\.captureHostRequestRecipients\(event\)\)/);
  ...
  assert.match(source, /prunePendingHostRecipients\(listener\)/);
});
...
test("pending host work tracks actual recipients rather than global subscriber count", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  assert.match(source, /interface PendingHostRequest/);
  assert.match(source, /legacyRecipients: Set<EventListener>/);
  assert.match(source, /compactRecipientIds: Set<number>/);
  assert.match(source, /captureHostRequestRecipients\(event\)/);
  assert.match(source, /prunePendingHostRecipients\(listener\)/);
  assert.match(source, /getSubscriberIds\(\)/);
});

test("destroy observers support both registry cleanup and live SSE teardown", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const onDestroy = source.slice(source.indexOf("onDestroy(cb:"), source.indexOf("/** Called when a session-changing command"));
  const destroy = source.slice(source.indexOf("async destroyAndWait()"));

  assert.match(onDestroy, /this\.destroyListeners\.push\(cb\)/);
  assert.match(onDestroy, /this\.destroyListeners\.splice\(index, 1\)/);
  assert.match(destroy, /this\.destroyListeners\.splice\(0\)/);
  assert.match(destroy, /for \(const listener of destroyListeners\)/);
});
