...
for (const lane of ["followUp", "steer"]) {
  for (const message of ["", "describe these images"]) {
    test(`${lane} dispatch preserves ${message ? "mixed text and" : "image-only"} images exactly once`, async (t) => {
      ...
      harness.emit({ type: "tool_execution_start", toolCallId: "image-tool" });
      await session.send({ type: "enqueue_message", lane, message, images });
      assert.deepEqual(outgoing(harness.commands), []);
      harness.emit({ type: "tool_execution_end", toolCallId: "image-tool" });
      await flush();
      if (lane === "followUp") {
        assert.deepEqual(outgoing(harness.commands), []);
      ...
      const expected = [{ type: "prompt", message, images, ...(lane === "steer" ? { streamingBehavior: "steer" } : {}) }];
      assert.deepEqual(outgoing(harness.commands), expected);
      assert.deepEqual((await session.send({ type: "get_message_queue" })).queue.items, []);
      harness.emit({ type: "agent_end", isTerminal: true });
      await flush();
      assert.deepEqual(outgoing(harness.commands), expected);
    });
...
test("image snapshots expose metadata only and recall returns images once", async (t) => {
  const { session, harness, events, WebRpcError } = await openSession(t);
  const images = [{ type: "image", mimeType: "image/png", data: "aGVsbG8=" }];
  harness.emit({ type: "agent_start" });
  const result = await session.send({ type: "enqueue_message", lane: "followUp", message: "", images });
  const item = result.queue.items[0];
  const expected = { ...item, attachments: [{ mimeType: "image/png", bytes: 5 }] };
  ...
  assert.deepEqual(state.messageQueue.items, [expected]);
  assert.deepEqual(listed.queue.items, [expected]);
  for (const event of [...events, ...replayed].filter((event) => event.type === "message_queue_update")) {
    assert.equal(JSON.stringify(event).includes(images[0].data), false);
    assert.equal(JSON.stringify(event).includes('"images"'), false);
  }
  assert.deepEqual(replayed.find((event) => event.type === "message_queue_update")?.queue.items, [expected]);
  assert.deepEqual(events.find((event) => event.type === "message_queue_update" && event.queue.items.length > 0)?.queue.items, [expected]);
  const recalled = await session.send({ type: "recall_queued_message", id: item.id, expectedRevision: result.queue.revision });
  assert.deepEqual(recalled.recalled.images, images);
  assert.equal(recalled.recalled.text, "");
  assert.deepEqual(recalled.queue.items, []);
  await assert.rejects(
    session.send({ type: "recall_queued_message", id: item.id, expectedRevision: recalled.queue.revision }),
    (error) => error instanceof WebRpcError && error.code === "queue_item_missing",
  );
  harness.emit({ type: "agent_end", isTerminal: true });
  await flush();
  assert.deepEqual(outgoing(harness.commands), []);
...
test("oversized relay recall leaves queued images available at the same revision", async (t) => {
  ...
  harness.emit({ type: "agent_start" });
  const result = await session.send({ type: "enqueue_message", lane: "followUp", message: "large recall", images });
  const command = {
  ...
  await assert.rejects(
    session.send({ ...command, __relayRecallMaxBytes: 15 * 1024 * 1024 }),
    (error) => error instanceof WebRpcError && error.code === "queue_recall_too_large",
  );
  assert.deepEqual((await session.send({ type: "get_message_queue" })).queue, result.queue);
  const recalled = await session.send(command);
  assert.deepEqual(recalled.recalled.images, images);
  assert.equal(recalled.recalled.text, "large recall");
  assert.deepEqual(recalled.queue.items, []);
  harness.emit({ type: "agent_end", isTerminal: true });
  await flush();
  assert.deepEqual(outgoing(harness.commands), []);
...
test("enqueue then delete before terminal yields no original outgoing prompt", async (t) => {
  ...
  const result = await session.send({
    type: "enqueue_message", lane: "followUp", message: "delete me",
...
test("failed send is retained and is not retried", async (t) => {
  ...
  const images = [{ type: "image", mimeType: "image/png", data: "aGVsbG8=" }];
  const result = await session.send({ type: "enqueue_message", lane: "followUp", message: "keep failed", images });
  assert.equal(result.queue.items[0].status, "failed");
  assert.deepEqual(result.queue.items[0].attachments, [{ mimeType: "image/png", bytes: 5 }]);
  assert.equal("images" in result.queue.items[0], false);
  const expected = [{ type: "prompt", message: "keep failed", images }];
  assert.deepEqual(outgoing(harness.commands), expected);
  harness.emit({ type: "agent_end", isTerminal: true });
  await flush();
  harness.emit({ type: "agent_end", isTerminal: true });
...
test("teardown and delayed settle cannot resurrect a discarded queue", async (t) => {
  ...
  const queued = session.send({
    type: "enqueue_message", lane: "followUp", message: "late",
    images: [{ type: "image", mimeType: "image/png", data: "aGVsbG8=" }],
  });
  await flush();
  assert.deepEqual(outgoing(harness.commands), []);
