...
test("project creation and unpack reject an escaped .omp before creating agents", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "omp-agent-escape-"));
  ...
  try {
    ...
    await fs.symlink(outside, path.join(cwd, ".omp"), "dir");
    globalThis.__piAllowedRootsCache = { roots: new Set([cwd, outside]), expiresAt: Date.now() + 60_000 };
    await assert.rejects(createAgent({ scope: "project", cwd, name: "helper", description: "", systemPrompt: "" }), /Access denied/);
    await assert.rejects(unpackBundledAgents({ scope: "project", cwd }), /Access denied/);
    await assert.rejects(discoverAgents(cwd), /Access denied/);
...
test("createAgent, updateAgent, and deleteAgent lifecycle in user directory", async () => {
  ...
  try {
    ...
    const createdYaml = parse(content.match(/^---\n([\s\S]*?)\n---/)[1]);
    assert.equal(createdYaml.scope, undefined);
    assert.equal(createdYaml.cwd, undefined);
    assert.equal((await fs.stat(createdPath)).mode & 0o777, 0o600);
    await fs.writeFile(createdPath, content.replace("\n---\n\n", "\ncustomSetting: keep\nscope: user\ncwd: /private\n---\n\n"));
    ...
    const updatedYaml = parse((await fs.readFile(createdPath, "utf8")).match(/^---\n([\s\S]*?)\n---/)[1]);
    assert.equal(updatedYaml.customSetting, "keep");
    for (const key of ["scope", "cwd", "source", "filePath", "isShadowed", "overrideModel", "prewalkOverride", "advisorOverride", "disabled"]) assert.equal(updatedYaml[key], undefined);
...
test("unpackBundledAgents unpacks markdown definitions", async () => {
  ...
  try {
    ...
    const scoutContent = await fs.readFile(path.join(res.targetDir, "scout.md"), "utf8");
    assert.match(scoutContent, /model:\s*"?@smol"?/);
    const taskContent = await fs.readFile(path.join(res.targetDir, "task.md"), "utf8");
    assert.match(taskContent, /model:\s*"?@task"?/);
  } finally {
    if (prevAgentDir !== undefined) {
      process.env.PI_CODING_AGENT_DIR = prevAgentDir;
    } else {
...
`;
    ...
    assert.equal(scout?.advisorOverride, "openai/advisor-model");
    ...
    assert.equal(reviewer?.prewalkOverride, false);
