import assert from "node:assert/strict";
import test from "node:test";
import { createJiti } from "jiti";

const jiti = createJiti(import.meta.url);
const {
  buildEmptyUsageResponse,
  canonicalizeUsageWindowId,
  fetchCachedUsage,
  fetchUsagePayload,
  getUsage,
  resolveUsageUsedFraction,
} = await jiti.import("./usage.ts");

test("resolveUsageUsedFraction: precedence branch 1 (usedFraction)", () => {
  const limit = {
    id: "test",
    label: "Test Limit",
    scope: { provider: "test" },
    amount: {
      usedFraction: 0.42,
      used: 10,
      limit: 100,
      unit: "requests",
      remainingFraction: 0.9,
    },
  };
  assert.equal(resolveUsageUsedFraction(limit), 0.42);
});

test("resolveUsageUsedFraction: precedence branch 2 (used / limit)", () => {
  const limit = {
    id: "test",
    label: "Test Limit",
    scope: { provider: "test" },
    amount: {
      used: 25,
      limit: 100,
      unit: "requests",
      remainingFraction: 0.9,
    },
  };
  assert.equal(resolveUsageUsedFraction(limit), 0.25);
});

test("resolveUsageUsedFraction: precedence branch 3 (unit percent)", () => {
  const limit = {
    id: "test",
    label: "Test Limit",
    scope: { provider: "test" },
    amount: {
      used: 75,
      unit: "percent",
      remainingFraction: 0.1,
    },
  };
  assert.equal(resolveUsageUsedFraction(limit), 0.75);
});

test("resolveUsageUsedFraction: precedence branch 4 (inverted remainingFraction)", () => {
  const limit = {
    id: "test",
    label: "Test Limit",
    scope: { provider: "test" },
    amount: {
      unit: "tokens",
      remainingFraction: 0.3,
    },
  };
  assert.equal(resolveUsageUsedFraction(limit), 0.7);
});

test("resolveUsageUsedFraction: fallback undefined", () => {
  const limit = {
    id: "test",
    label: "Test Limit",
    scope: { provider: "test" },
    amount: {
      unit: "unknown",
    },
  };
  assert.equal(resolveUsageUsedFraction(limit), undefined);
});

test("buildEmptyUsageResponse creates proper structure", () => {
  const res = buildEmptyUsageResponse("no-credentials");
  assert.equal(res.cached, false);
  assert.deepEqual(res.reports, []);
  assert.deepEqual(res.accountsWithoutUsage, []);
  assert.deepEqual(res.disabledCredentials, []);
  assert.deepEqual(res.capacity, {});
  assert.equal(res.emptyReason, "no-credentials");
  assert.equal(typeof res.generatedAt, "number");
});

test("fetchUsagePayload: success with valid payload", async () => {
  const mockPayload = {
    generatedAt: 1234567890,
    reports: [
      {
        provider: "mock",
        fetchedAt: 1234567800,
        limits: [],
      },
    ],
    accountsWithoutUsage: [],
    disabledCredentials: [],
    capacity: {},
  };

  const executor = async () => ({
    stdout: JSON.stringify(mockPayload),
  });

  const result = await fetchUsagePayload({ executor });
  assert.equal(result.ok, true);
  if (result.ok) {
    assert.equal(result.payload.cached, false);
    assert.equal(result.payload.reports.length, 1);
    assert.equal(result.payload.emptyReason, undefined);
  }
});

test("fetchUsagePayload: accounts without usage do not imply an empty reason", async () => {
  const mockPayload = {
    generatedAt: 1234567890,
    reports: [],
    accountsWithoutUsage: [{ provider: "ollama", type: "api_key" }],
    disabledCredentials: [],
    capacity: {},
  };

  const executor = async () => ({
    stdout: JSON.stringify(mockPayload),
  });

  const result = await fetchUsagePayload({ executor });
  assert.equal(result.ok, true);
  if (result.ok) {
    assert.equal(result.payload.emptyReason, undefined);
  }
});

test("fetchUsagePayload: all empty -> emptyReason no-credentials", async () => {
  const mockPayload = {
    generatedAt: 1234567890,
    reports: [],
    accountsWithoutUsage: [],
    disabledCredentials: [],
    capacity: {},
  };

  const executor = async () => ({
    stdout: JSON.stringify(mockPayload),
  });

  const result = await fetchUsagePayload({ executor });
  assert.equal(result.ok, true);
  if (result.ok) {
    assert.equal(result.payload.emptyReason, "no-credentials");
  }
});

test("fetchUsagePayload: legacy no-usage-endpoint reason is normalized", async () => {
  const executor = async () => ({
    stdout: JSON.stringify({
      generatedAt: 1234567890,
      reports: [],
      accountsWithoutUsage: [],
      disabledCredentials: [],
      capacity: {},
      emptyReason: "no-usage-endpoint",
    }),
  });

  const result = await fetchUsagePayload({ executor });
  assert.equal(result.ok, true);
  if (result.ok) {
    assert.equal(result.payload.emptyReason, "no-credentials");
  }
});

test("fetchCachedUsage: refresh supersedes an ordinary in-flight fetch", async () => {
  let resolveOrdinary;
  const ordinaryPayload = {
    generatedAt: 1,
    reports: [],
    accountsWithoutUsage: [],
    disabledCredentials: [],
    capacity: {},
    cached: false,
    emptyReason: "no-credentials",
  };
  const refreshedPayload = { ...ordinaryPayload, generatedAt: 2 };
  const ordinaryResult = new Promise((resolve) => {
    resolveOrdinary = resolve;
  });
  let fetchCount = 0;
  let invalidateCount = 0;
  const dependencies = {
    ttlMs: 60_000,
    invalidate: async () => {
      invalidateCount += 1;
    },
    fetch: async () => {
      fetchCount += 1;
      if (fetchCount === 1) return ordinaryResult;
      assert.equal(invalidateCount, 1);
      return { ok: true, payload: refreshedPayload };
    },
  };
  const state = { entry: { at: 0, payload: ordinaryPayload } };

  const ordinaryRequest = fetchCachedUsage(state, false, dependencies);
  const refreshRequest = fetchCachedUsage(state, true, dependencies);
  const concurrentRefresh = fetchCachedUsage(state, true, dependencies);
  const requestDuringRefresh = fetchCachedUsage(state, false, dependencies);

  const [refreshResult, concurrentResult, duringResult] = await Promise.all([
    refreshRequest,
    concurrentRefresh,
    requestDuringRefresh,
  ]);
  resolveOrdinary({ ok: true, payload: ordinaryPayload });
  const ordinaryResponse = await ordinaryRequest;

  assert.equal(invalidateCount, 1);
  assert.equal(fetchCount, 2);
  assert.equal(refreshResult.payload.generatedAt, 2);
  assert.equal(concurrentResult.payload.generatedAt, 2);
  assert.equal(duringResult.payload.generatedAt, 2);
  assert.equal(ordinaryResponse.payload.generatedAt, 2);
  assert.equal(state.entry.payload.generatedAt, 2);
});

test("shared usage readers deduplicate, refresh together, and isolate profile switches", async (t) => {
  const keys = ["PI_CODING_AGENT_DIR", "OMP_PROFILE", "PI_PROFILE"];
  const previous = keys.map((key) => process.env[key]);
  t.after(() => keys.forEach((key, index) => {
    if (previous[index] === undefined) delete process.env[key];
    else process.env[key] = previous[index];
  }));
  delete process.env.OMP_PROFILE;
  delete process.env.PI_PROFILE;
  process.env.PI_CODING_AGENT_DIR = "/tmp/ompgui-usage-shared-a";
  const first = Promise.withResolvers();
  const invalidation = Promise.withResolvers();
  const payload = { ...buildEmptyUsageResponse(), generatedAt: 1 };
  let reads = 0;
  let invalidations = 0;
  let now = 0;
  const dependencies = {
    now: () => now,
    fetch: async () => {
      reads += 1;
      return reads === 1 ? first.promise : { ok: true, payload: { ...payload, generatedAt: reads } };
    },
    invalidate: async () => {
      invalidations += 1;
      await invalidation.promise;
    },
  };
  const restRead = getUsage(false, dependencies);
  const relayRead = getUsage(false, dependencies);
  assert.equal(reads, 1);
  first.resolve({ ok: true, payload });
  assert.deepEqual(await restRead, await relayRead);
  now = 59_999;
  assert.equal((await getUsage(false, dependencies)).payload.cached, true);
  assert.equal(reads, 1);
  now = 60_000;
  assert.equal((await getUsage(false, dependencies)).payload.generatedAt, 2);

  const restRefresh = getUsage(true, dependencies);
  const relayRefresh = getUsage(true, dependencies);
  const legacyRead = getUsage(false, dependencies);
  assert.equal(invalidations, 1);
  invalidation.resolve();
  for (const result of await Promise.all([restRefresh, relayRefresh, legacyRead])) {
    assert.equal(result.payload.generatedAt, 3);
  }
  assert.equal((await getUsage(false, dependencies)).payload.generatedAt, 3);
  assert.equal(reads, 3);

  // An old profile's request must not become the new profile's cached payload.
  now += 60_000;
  const stale = Promise.withResolvers();
  const oldProfileRead = getUsage(false, { ...dependencies, fetch: () => stale.promise });
  process.env.OMP_PROFILE = "ompgui-usage-shared-profile";
  const newProfileRead = await getUsage(false, dependencies);
  assert.equal(newProfileRead.payload.generatedAt, 4);
  stale.resolve({ ok: true, payload });
  await oldProfileRead;
  assert.equal((await getUsage(false, dependencies)).payload.generatedAt, 4);
  delete process.env.OMP_PROFILE;
  process.env.PI_CODING_AGENT_DIR = "/tmp/ompgui-usage-shared-b";
  assert.equal((await getUsage(false, dependencies)).payload.generatedAt, 5);
});

test("fetchUsagePayload: unknown command rejection -> 501", async () => {
  const executor = async () => {
    throw new Error("error: unknown command 'usage'");
  };

  const result = await fetchUsagePayload({ executor });
  assert.equal(result.ok, false);
  if (!result.ok) {
    assert.equal(result.status, 501);
  }
});

test("fetchUsagePayload: invalid JSON stdout -> 501", async () => {
  const executor = async () => ({
    stdout: "Some non-json text output",
  });

  const result = await fetchUsagePayload({ executor });
  assert.equal(result.ok, false);
  if (!result.ok) {
    assert.equal(result.status, 501);
  }
});

test("fetchUsagePayload: execution failure -> 502", async () => {
  const executor = async () => {
    throw new Error("Network timeout contacting auth server");
  };

  const result = await fetchUsagePayload({ executor });
  assert.equal(result.ok, false);
  if (!result.ok) {
    assert.equal(result.status, 502);
    assert.match(result.error, /Network timeout/);
  }
});

test("canonicalizeUsageWindowId: daily -> 5h", () => {
  assert.equal(canonicalizeUsageWindowId("daily"), "5h");
});

test("canonicalizeUsageWindowId: 30d -> monthly", () => {
  assert.equal(canonicalizeUsageWindowId("30d"), "monthly");
});

test("canonicalizeUsageWindowId: 24h duration -> 5h", () => {
  assert.equal(canonicalizeUsageWindowId(undefined, 24 * 60 * 60 * 1000), "5h");
});

test("canonicalizeUsageWindowId: 5h duration -> 5h", () => {
  assert.equal(canonicalizeUsageWindowId(undefined, 5 * 60 * 60 * 1000), "5h");
});

test("canonicalizeUsageWindowId: unknown undefined", () => {
  assert.equal(canonicalizeUsageWindowId("weekly", 3 * 60 * 60 * 1000), undefined);
});

test("fetchUsagePayload: google-antigravity daily 24h rewrites to 5h and stays in reports", async () => {
  const mockPayload = {
    generatedAt: 1234567890,
    reports: [
      {
        provider: "google-antigravity",
        fetchedAt: 1234567800,
        limits: [
          {
            id: "quota",
            label: "Quota",
            scope: { provider: "google-antigravity", windowId: "daily" },
            window: {
              id: "daily",
              label: "daily",
              durationMs: 24 * 60 * 60 * 1000,
            },
            amount: { unit: "percent", used: 100 },
          },
        ],
      },
    ],
    accountsWithoutUsage: [],
    disabledCredentials: [],
    capacity: {},
  };

  const result = await fetchUsagePayload({
    executor: async () => ({ stdout: JSON.stringify(mockPayload) }),
  });
  assert.equal(result.ok, true);
  if (!result.ok) return;
  assert.equal(result.payload.reports.length, 1);
  const limit = result.payload.reports[0].limits[0];
  assert.equal(limit.scope.windowId, "5h");
  assert.equal(limit.window.id, "5h");
  assert.equal(limit.window.label, "5h");
  assert.equal(limit.window.durationMs, 24 * 60 * 60 * 1000);
});

test("fetchUsagePayload: capacity daily + 5h merge into one 5h entry", async () => {
  const mockPayload = {
    generatedAt: 1234567890,
    reports: [
      {
        provider: "google-antigravity",
        fetchedAt: 1234567800,
        limits: [],
      },
    ],
    accountsWithoutUsage: [],
    disabledCredentials: [],
    capacity: {
      "google-antigravity": [
        {
          window: "daily",
          durationMs: 24 * 60 * 60 * 1000,
          accounts: 2,
          usedAccounts: 1,
          remainingAccounts: 1,
        },
        {
          window: "5h",
          durationMs: 5 * 60 * 60 * 1000,
          accounts: 3,
          usedAccounts: 2,
          remainingAccounts: 1,
        },
      ],
      anthropic: [
        {
          window: "7d",
          durationMs: 7 * 24 * 60 * 60 * 1000,
          accounts: 1,
          usedAccounts: 0,
          remainingAccounts: 1,
        },
      ],
    },
  };

  const result = await fetchUsagePayload({
    executor: async () => ({ stdout: JSON.stringify(mockPayload) }),
  });
  assert.equal(result.ok, true);
  if (!result.ok) return;
  assert.deepEqual(result.payload.capacity["google-antigravity"], [
    {
      window: "5h",
      durationMs: 5 * 60 * 60 * 1000,
      accounts: 5,
      usedAccounts: 3,
      remainingAccounts: 2,
    },
  ]);
  assert.deepEqual(result.payload.capacity.anthropic, [
    {
      window: "7d",
      durationMs: 7 * 24 * 60 * 60 * 1000,
      accounts: 1,
      usedAccounts: 0,
      remainingAccounts: 1,
    },
  ]);
});
