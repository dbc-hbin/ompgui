...
test("frame-gated readiness waits for markConnected after transport open", async () => {
  const { createEventStreamConnectionManager } = await loadSubject();
  const sources = [];
  const manager = createEventStreamConnectionManager({
    timeoutMs: 60_000,
  ...
  manager.markConnected("session-1", sources[0]);
  assert.deepEqual(await readiness, { status: "connected", source: sources[0] });
});
