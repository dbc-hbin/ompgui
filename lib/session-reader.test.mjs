...
import { appendFileSync, mkdirSync, mkdtempSync, readFileSync, realpathSync, rmSync, symlinkSync, unlinkSync, utimesSync, writeFileSync } from "node:fs";
...
const { getDisplayableAssistantBlocks } = await jiti.import("./message-display.ts");
const {
  buildSessionContext,
  entryToUiMessage,
  cacheSessionPath,
...
const { listAllSessionInfos, loadSessionFile, materializeSessionEntries, parseJsonlLenient, readSessionEntryByIdSync } = await jiti.import("./omp/session-files.ts");
...
test("skips shape-malformed message entries while building context", () => {
  const entries = [
    ...
    },
    {
      type: "message",
      id: "bad-missing-role",
      parentId: "bad-null-message",
      timestamp: "2026-01-01T00:00:02.000Z",
      message: { content: "not a valid agent message" },
    },
    {
      type: "message",
      id: "bad-model-shape",
      parentId: "bad-missing-role",
      timestamp: "2026-01-01T00:00:03.000Z",
      message: { role: "assistant", provider: {}, model: [], content: null },
    },
    assistantEntry("a1", "bad-model-shape", "done"),
  ];

  assert.equal(entryToUiMessage(entries[1], {}), null);
  assert.equal(entryToUiMessage(entries[2], {}), null);
  const context = buildSessionContext(entries);
  assert.deepEqual(context.entryIds, ["u1", "a1"]);
  assert.deepEqual(context.model, { provider: "test", modelId: "test-model" });
});

test("uses the newest valid todo snapshot and falls back past invalid updates", () => {
  const pending = [{ name: "Work", tasks: [{ content: "Ship", status: "pending" }] }];
  const completed = [{ name: "Work", tasks: [{ content: "Ship", status: "completed" }] }];
  const entries = [
    { type: "custom", id: "old", parentId: null, customType: "user_todo_edit", data: { phases: pending } },
    { type: "message", id: "new", parentId: "old", message: { role: "toolResult", toolName: "todo", content: [], details: { phases: completed } } },
  ];
  assert.deepEqual(getTodoPhasesFromEntries(entries), completed);
  entries.push({ type: "custom", id: "invalid", parentId: "new", customType: "user_todo_edit", data: { phases: [{ name: "Bad", tasks: null }] } });
  entries.push({ type: "message", id: "failed", parentId: "invalid", message: { role: "toolResult", toolName: "todo", isError: true, content: [], details: { phases: pending } } });
  assert.deepEqual(getTodoPhasesFromEntries(entries), completed);
  entries.push({ type: "custom", id: "clear", parentId: "failed", customType: "user_todo_edit", data: { phases: [] } });
  assert.deepEqual(getTodoPhasesFromEntries(entries), []);
  assert.deepEqual(getTodoPhasesFromEntries([{ type: "custom", id: "cycle", parentId: "cycle" }]), []);
});
...
test("renders legacy assistant string content with thinking deferral", () => {
  const entries = [
    userEntry("u1", null, "start"),
    {
      type: "message",
  ...
  assert.deepEqual(getDisplayableAssistantBlocks(context.messages[1]), [
    { type: "text", text: "legacy plain-text answer" },
  ]);
});
...
test("defers only base64 images from historical tool results", () => {
  ...
  assert.deepEqual(deferred.messages[2].deferredImages, { entryId: "tr1", count: 2 });
  // Deferred payloads carry no base64 image bytes.
  assert.equal(JSON.stringify(deferred.messages[2]).includes("QUJDRA=="), false);
  ...
  assert.equal(full.messages[2].deferredImages, undefined);
});

test("merges generated details.images into full history and defers them lazily", () => {
  const generated = { data: "R0VO", mimeType: "image/png" };
  const entries = [
    {
      type: "message",
      id: "tr1",
      ...
      message: {
        ...
        details: { images: [generated, { ...generated }] },
      },
    },
  ];

  const full = buildSessionContext(entries);
  assert.deepEqual(full.messages[0].content, [
    { type: "text", text: "generated" },
    { type: "image", data: "R0VO", mimeType: "image/png" },
  ]);
  assert.equal(full.messages[0].details, undefined);

  const deferred = buildSessionContext(entries, undefined, { deferToolResultImages: true });
  // Details-only images leave content untouched (no omission placeholder —
  // that path only covers content blocks) but still attach the lazy marker.
  assert.deepEqual(deferred.messages[0].content, [{ type: "text", text: "generated" }]);
  assert.deepEqual(deferred.messages[0].deferredImages, { entryId: "tr1", count: 1 });
  assert.equal(JSON.stringify(deferred.messages[0]).includes("R0VO"), false);

  // Malformed tool results project without crashing and carry no marker.
  const malformed = entryToUiMessage({
    type: "message",
    id: "bad",
    parentId: null,
    timestamp: "2026-01-01T00:00:00.000Z",
    message: { role: "toolResult", toolCallId: "c", content: "bad", details: { images: "bad" } },
  }, { deferToolResultImages: true });
  assert.equal(malformed.content, "bad");
  assert.equal(malformed.deferredImages, undefined);
});

test("loads one tool result entry with blob materialization", async () => {
  const { getToolResultImagesForEntry } = await jiti.import("./session-reader.ts");
  await withAgentDir(async (dir) => {
    const mediaBytes = Buffer.from("media-bytes");
    const hash = createHash("sha256").update(mediaBytes).digest("hex");
    const blobDir = join(process.env.PI_CODING_AGENT_DIR, "blobs");
    mkdirSync(blobDir, { recursive: true });
    writeFileSync(join(blobDir, hash), mediaBytes);
    const blobRef = `blob:sha256:${hash}`;
    const filePath = writeSessionFile(dir, "2026-01-01_media.jsonl", {
      id: "media",
      cwd: dir,
      timestamp: "2026-01-01T00:00:00.000Z",
    }, [
      {
        ...
        message: {
          ...
          toolCallId: "call-1",
          toolName: "read",
          content: [{ type: "image", data: blobRef, mimeType: "image/png" }],
          details: { images: [{ data: Buffer.from("media-bytes").toString("base64"), mimeType: "image/png" }] },
        },
      },
      userEntry("u1", "t1", "next"),
    ]);

    const expected = mediaBytes.toString("base64");
    assert.deepEqual(getToolResultImagesForEntry(filePath, "t1"), {
      images: [{ type: "image", data: expected, mimeType: "image/png" }],
      missingCount: 0,
    });
    // Missing ids, non-toolResult entries, and malformed ids return null.
    assert.equal(getToolResultImagesForEntry(filePath, "missing"), null);
    assert.equal(getToolResultImagesForEntry(filePath, "u1"), null);
    assert.equal(getToolResultImagesForEntry(join(dir, "gone.jsonl"), "t1"), null);
    // The shared raw document keeps the unresolved blob ref.
    assert.equal(getSessionDocument(filePath).entries[0].message.content[0].data, blobRef);
  });
});

test("deferred media succeeds for inline images and 404s for missing or non-image entries", async () => {
  const { getToolResultImagesForEntry } = await jiti.import("./session-reader.ts");
  await withAgentDir(async (dir) => {
    const filePath = writeSessionFile(dir, "2026-01-01_media-inline.jsonl", {
      id: "media-inline",
      cwd: dir,
      timestamp: "2026-01-01T00:00:00.000Z",
    }, [
      {
        ...
        message: {
          ...
          details: { images: [{ data: "RUZH", mimeType: "image/jpeg" }] },
        },
      },
      {
        type: "message",
    ...
    assert.equal(getToolResultImagesForEntry(filePath, "t2"), null);
    assert.equal(getToolResultImagesForEntry(filePath, "missing"), null);
    assert.equal(getToolResultImagesForEntry(filePath, "u1"), null);
  });
});

test("deferred media rejects image counts and byte totals before reading payloads", async () => {
  const { getToolResultImagesForEntry } = await jiti.import("./session-reader.ts");
  const attachments = await jiti.import("./image-attachments.ts");
  await withAgentDir(async (dir) => {
    const blobDir = join(process.env.PI_CODING_AGENT_DIR, "blobs");
    mkdirSync(blobDir, { recursive: true });
    const smallHash = "c".repeat(64);
    writeFileSync(join(blobDir, smallHash), Buffer.from("unrelated-bytes"));
    const bigHash = "d".repeat(64);
    writeFileSync(join(blobDir, bigHash), Buffer.alloc(attachments.MAX_TOOL_RESULT_IMAGE_BYTES + 1));

    const image = { type: "image", data: "QUJD", mimeType: "image/png" };
    const filePath = writeSessionFile(dir, "2026-01-01_media-limits.jsonl", {
      id: "media-limits",
      cwd: dir,
      timestamp: "2026-01-01T00:00:00.000Z",
    }, [
      {
        ...
        message: {
          ...
          toolName: "read",
          content: Array.from({ length: attachments.MAX_TOOL_RESULT_IMAGES + 1 }, (_, index) => ({ ...image, data: `QUJD${index}` })),
        },
      },
      {
        type: "message",
        ...
        message: {
          ...
          toolName: "read",
          content: [{ type: "image", data: "QUJD", mimeType: "image/png" }],
          details: { images: [{ data: "A".repeat(16 * 1024 * 1024), mimeType: "image/png" }] },
        },
      },
      {
        type: "message",
        ...
        message: {
          role: "toolResult",
          toolCallId: "call-blob",
          toolName: "read",
          content: [{ type: "image", data: `blob:sha256:${bigHash}`, mimeType: "image/png" }],
        },
      },
      {
        type: "message",
        ...
        message: {
          role: "toolResult",
          toolCallId: "call-total",
          toolName: "read",
          content: Array.from({ length: 4 }, (_, index) => ({ type: "image", data: "A".repeat(9 * 1024 * 1024), mimeType: `image/${index}` })),
        },
      },
      {
        type: "message",
        ...
        message: {
          ...
          result: `blob:sha256:${smallHash}`,
          image_url: `blob:sha256:${smallHash}`,
          content: [{ type: "image", data: "QUJD", mimeType: "image/png" }],
          details: {
    ...
    const isTooLarge = (error) => error?.code === attachments.TOOL_RESULT_IMAGES_TOO_LARGE_CODE;
    // Count and aggregate violations throw synchronously before any read.
    assert.throws(() => getToolResultImagesForEntry(filePath, "many"), isTooLarge);
    assert.throws(() => getToolResultImagesForEntry(filePath, "big-inline"), isTooLarge);
    assert.throws(() => getToolResultImagesForEntry(filePath, "big-total"), isTooLarge);

    // The oversized blob rejects from its stat size before selective
    // materialization can read its payload.
    assert.throws(() => getToolResultImagesForEntry(filePath, "big-blob"), isTooLarge);
    // Unrelated refs at message level and inside other details keys are never
    // resolved: deleting the small blob leaves the valid inline image intact.
    unlinkSync(join(blobDir, smallHash));
    assert.deepEqual(getToolResultImagesForEntry(filePath, "unrelated"), {
...
test("deferred media reports missing blobs while preserving surviving images", async () => {
  const { getToolResultImagesForEntry } = await jiti.import("./session-reader.ts");
  await withAgentDir(async (dir) => {
    const blobDir = join(process.env.PI_CODING_AGENT_DIR, "blobs");
    mkdirSync(blobDir, { recursive: true });
    const presentHash = "e".repeat(64);
    const missingHash = "f".repeat(64);
    writeFileSync(join(blobDir, presentHash), Buffer.from("present-image"));

    const filePath = writeSessionFile(dir, "2026-01-01_media-partial.jsonl", {
      id: "media-partial",
      cwd: dir,
      timestamp: "2026-01-01T00:00:00.000Z",
    }, [{
      ...
      message: {
        ...
        content: [
          { type: "image", data: `blob:sha256:${presentHash}`, mimeType: "image/png" },
          { type: "image", data: `blob:sha256:${missingHash}`, mimeType: "image/jpeg" },
        ],
      },
    }]);

    assert.deepEqual(getToolResultImagesForEntry(filePath, "partial"), {
      images: [{
        type: "image",
        data: Buffer.from("present-image").toString("base64"),
        mimeType: "image/png",
      }],
      missingCount: 1,
    });
...
test("groups symlink workspace sessions by canonical root without rewriting cwd", async () => {
  await withAgentDir(async (dir) => {
    for (const kind of ["non-git", "unborn-git"]) {
      const workspace = join(process.env.PI_CODING_AGENT_DIR, kind);
      const alias = `${workspace}-alias`;
      mkdirSync(workspace);
      if (kind === "unborn-git") execFileSync("git", ["init", "--quiet", workspace]);
      symlinkSync(workspace, alias, process.platform === "win32" ? "junction" : "dir");
      const canonical = realpathSync(workspace);
      writeSessionFile(dir, `${kind}-alias.jsonl`, { id: `${kind}-alias`, cwd: alias }, [userEntry("u1", null, "alias")]);
      writeSessionFile(dir, `${kind}-real.jsonl`, { id: `${kind}-real`, cwd: canonical }, [userEntry("u1", null, "real")]);
      invalidateSessionListCache();
      const sessions = await listAllSessions();
      const aliased = sessions.find((session) => session.id === `${kind}-alias`);
      const direct = sessions.find((session) => session.id === `${kind}-real`);
      assert.equal(aliased?.cwd, alias);
      assert.equal(aliased?.projectRoot, canonical);
      assert.equal(aliased?.projectKey, direct?.projectKey);
      assert.equal(aliased?.worktreeBranch, undefined);
    }
  });
});

test("skips non-object and torn JSONL lines without losing surrounding records", async () => {
  await withAgentDir(async (dir) => {
    const entries = [userEntry("before", null, "before"), userEntry("after", "before", "after")];
    const filePath = writeSessionFile(dir, "malformed.jsonl", { id: "malformed", cwd: dir }, []);
    const invalid = ["null", "[]", "42", "true", '"text"', '{"torn":'];
    appendFileSync(filePath, [JSON.stringify(entries[0]), ...invalid, JSON.stringify(entries[1])].join("\n") + "\n");
    assert.deepEqual(parseJsonlLenient(readFileSync(filePath, "utf8")).slice(1), entries);
    assert.deepEqual(loadSessionFile(filePath).entries, entries);
    assert.deepEqual(readSessionEntryByIdSync(filePath, "after"), entries[1]);
    assert.deepEqual(buildSessionContext(getSessionEntries(filePath)).entryIds, ["before", "after"]);
    assert.equal((await listAllSessionInfos()).find((session) => session.id === "malformed")?.id, "malformed");
  });
});
...
test("shares one raw session document until the file identity changes", () => {
  const dir = mkdtempSync(join(tmpdir(), "omp-web-document-cache-"));
  const filePath = writeSessionFile(dir, "session.jsonl", {
    id: "cached",
    cwd: dir,
    timestamp: "2026-01-01T00:00:00.000Z",
  ...
  try {
    const first = getSessionDocument(filePath);
    const second = getSessionDocument(filePath);
    assert.strictEqual(second, first);
    assert.strictEqual(second.entries, first.entries);
    assert.equal(Object.isFrozen(first), true);
    assert.equal(Object.isFrozen(first.entries), true);
    assert.equal(Object.isFrozen(first.entries[0].message), true);
    assert.throws(() => {
      first.entries[0].message.content = "contaminated";
    }, TypeError);
    assert.equal(second.entries[0].message.content, "first");

    appendFileSync(filePath, `${JSON.stringify(userEntry("u2", "u1", "second"))}\n`);
    const afterAppend = getSessionDocument(filePath);
    assert.notStrictEqual(afterAppend, first);
    assert.equal(afterAppend.entries.length, 2);

    invalidateSessionListCache();
    const afterExplicitInvalidation = getSessionDocument(filePath);
    assert.notStrictEqual(afterExplicitInvalidation, afterAppend);
    assert.equal(afterExplicitInvalidation.entries.length, 2);
  } finally {
    invalidateSessionListCache();
    rmSync(dir, { recursive: true, force: true });
  }
...
test("materializes blob entries without contaminating the shared raw document", async () => {
  await withAgentDir(async (dir) => {
    const hash = "a".repeat(64);
    const blobDir = join(process.env.PI_CODING_AGENT_DIR, "blobs");
    mkdirSync(blobDir, { recursive: true });
    writeFileSync(join(blobDir, hash), Buffer.from("image-bytes"));
    const blobRef = `blob:sha256:${hash}`;
    const filePath = writeSessionFile(dir, "2026-01-01_blobs.jsonl", {
      id: "blobs",
      cwd: dir,
      timestamp: "2026-01-01T00:00:00.000Z",
    }, [
    ...
    const full = materializeSessionEntries(document.entries);
    const deferred = materializeSessionEntries(document.entries, { skipToolResultImages: true });
    const expectedBase64 = Buffer.from("image-bytes").toString("base64");

    assert.equal(document.entries[0].message.content[0].data, blobRef);
    assert.equal(document.entries[1].message.content[0].data, blobRef);
    assert.equal(full[0].message.content[0].data, expectedBase64);
    assert.equal(full[1].message.content[0].data, expectedBase64);
    assert.equal(deferred[0].message.content[0].data, expectedBase64);
    assert.equal(deferred[1].message.content[0].data, blobRef);
    assert.notStrictEqual(full[0], document.entries[0]);
    assert.strictEqual(deferred[1], document.entries[1]);
  });
});
...
test("reuses the file-list cache when a subdirectory is newer than root", async () => {
  await withAgentDir(async (dir) => {
    const sessionsRoot = join(dir, "..");
    const laterProjectDir = join(sessionsRoot, "-later-project");
    mkdirSync(laterProjectDir);
    writeSessionFile(dir, "2026-01-01_first.jsonl", {
      id: "first",
      cwd: join(tmpdir(), "omp-web-cache-project"),
      timestamp: "2026-01-01T00:00:00.000Z",
    }, [userEntry("u1", null, "first")]);
    writeSessionFile(laterProjectDir, "2026-01-02_second.jsonl", {
      id: "second",
      cwd: join(tmpdir(), "omp-web-cache-project"),
      timestamp: "2026-01-02T00:00:00.000Z",
    }, [userEntry("u1", null, "second")]);

    // Keep the root older than both project directories so this exercises the
    // NTFS path where a subdirectory mtime exceeds the root mtime.
    utimesSync(sessionsRoot, new Date(1_000), new Date(1_000));
    utimesSync(dir, new Date(2_000), new Date(2_000));
    utimesSync(laterProjectDir, new Date(2_000), new Date(2_000));

    const first = await listAllSessionInfos();
    assert.equal(first.length, 2);
    const firstCache = globalThis.__ompSessionFileListCache?.get(sessionsRoot);
    assert.ok(firstCache);
    assert.ok(firstCache.subMaxMtimeMs > firstCache.rootMtimeMs);
    const cachedFiles = firstCache.files;

    await listAllSessionInfos();
    assert.strictEqual(globalThis.__ompSessionFileListCache?.get(sessionsRoot)?.files, cachedFiles);
    await listAllSessionInfos();
    assert.strictEqual(globalThis.__ompSessionFileListCache?.get(sessionsRoot)?.files, cachedFiles);

    writeSessionFile(laterProjectDir, "2026-01-03_third.jsonl", {
      id: "third",
      cwd: join(tmpdir(), "omp-web-cache-project"),
      timestamp: "2026-01-03T00:00:00.000Z",
    }, [userEntry("u1", null, "third")]);
    utimesSync(laterProjectDir, new Date(3_000), new Date(3_000));
    utimesSync(sessionsRoot, new Date(1_000), new Date(1_000));

    const after = await listAllSessionInfos();
    assert.equal(after.some((session) => session.id === "third"), true);
    assert.notStrictEqual(globalThis.__ompSessionFileListCache?.get(sessionsRoot)?.files, cachedFiles);
  });
});
