...
import { appendFileSync, mkdirSync, mkdtempSync, readFileSync, rmSync, unlinkSync, utimesSync, writeFileSync } from "node:fs";
...
const { listAllSessionInfos, materializeSessionEntries } = await jiti.import("./omp/session-files.ts");
const sessionReaderSource = readFileSync(new URL("./session-reader.ts", import.meta.url), "utf8");
...
test("deferred media scans only the requested v3 entry", () => {
  const implementation = sessionReaderSource.match(
    /export function getToolResultImagesForEntry\([\s\S]*?\n\}/,
  )?.[0] ?? "";
  assert.match(implementation, /readSessionEntryByIdSync\(filePath, entryId\)/);
  assert.doesNotMatch(implementation, /getSessionDocument\(/);
});
