...
test("queue revision guards ignore stale events and keep resync fetches", async () => {
  const { matchesQueueRevision } = await import("./session-load-fence.ts");
  assert.equal(matchesQueueRevision("session-a", "session-a", 8, 8, 1, 1, -1, 0, "fetch"), true);
  assert.equal(matchesQueueRevision("session-a", "session-a", 8, 8, 1, 1, 4, 4, "fetch"), true);
  assert.equal(matchesQueueRevision("session-a", "session-a", 8, 8, 1, 1, 5, 4, "fetch"), false);
  assert.equal(matchesQueueRevision("session-a", "session-a", 8, 8, 1, 1, 4, 4, "event"), false);
  assert.equal(matchesQueueRevision("session-a", "session-a", 8, 8, 1, 1, 4, 5, "event"), true);
  assert.equal(matchesQueueRevision("session-a", "session-a", 9, 8, 1, 1, 4, 6, "event"), false);
  assert.equal(matchesQueueRevision("session-b", "session-a", 8, 8, 1, 1, 4, 6, "fetch"), false);
  assert.equal(matchesQueueRevision("session-a", "session-a", 8, 8, 2, 1, -1, 9, "fetch"), false);
  assert.equal(matchesQueueRevision("session-a", "session-a", 8, 8, 2, 2, -1, 0, "event"), true);
});
