...
test("persists and reads by origin and session without exposing runtime authority", () => {
  ...
  globalThis.window = {
    ...
    OmpguiRemoteReplica: {
      ...
      getSnapshot() { return nativeJson; },
      getOrigin() { return "https://omp.example.test"; },
    },
  };
  try {
    const snapshot = projectRemoteReplica({
    ...
    assert.deepEqual(loadRemoteReplicaSnapshot("session-adapter"), snapshot);
    assert.equal(loadRemoteReplicaSnapshot("other-session"), null);
  } finally {
