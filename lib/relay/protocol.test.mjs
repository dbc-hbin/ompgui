...
test("parses scoped settings snapshot and update frames", () => {
  const get = parseClientFrame(JSON.stringify({ op: "settings.get", scope: "project", cwd: "/workspace" }));
  assert.deepEqual(get, { op: "settings.get", scope: "project", cwd: "/workspace" });

  const update = parseClientFrame(JSON.stringify({
    op: "settings.update",
    scope: "global",
    settings: { retry: { enabled: false } },
    reset: ["retry.maxRetries"],
    secrets: { "auth.broker.token": null },
    confirm: ["auth.broker.token"],
  }));
  ...
  assert.equal(parseClientFrame(JSON.stringify({ op: "settings.update", settings: "dark" })).code, "invalid_settings");
  assert.equal(parseClientFrame(JSON.stringify({ op: "settings.update", reset: [1] })).code, "invalid_settings");
  assert.deepEqual(parseClientFrame(JSON.stringify({ op: "settings.update" })), { op: "settings.update" });
});
