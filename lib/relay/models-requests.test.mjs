...
test("registry relay exposes and protects path-scoped native rules", async () => {
  const restore = isolateAgentDir();
  try {
    const settingsPath = join(process.env.PI_CODING_AGENT_DIR, "config.yml");
    writeFileSync(settingsPath, "disabledProviders:\n  - anthropic\n  - pathPrefix: /tmp/project\n    providers: [openai]\n", "utf8");
    const read = await handleModelsRequest("registry.get", {}, ctx);
    assert.deepEqual(read.settings.disabledProviders, ["anthropic"]);
    assert.equal(read.settings.registryHasScopedEntries, true);
    await throwsCode(
      () => handleModelsRequest("registry.set", { disabledProviders: ["google"] }, ctx),
      "registry_write_failed",
    );
    assert.match(readFileSync(settingsPath, "utf8"), /pathPrefix: \/tmp\/project/);
  } finally {
    restore();
  }
});
...
test("configuration verification validates both transports without native startup or network access", async (t) => {
  const childProcess = jiti("node:child_process");
  const spawn = t.mock.method(childProcess, "spawn", () => { throw new Error("native startup forbidden"); });
  ...
  const candidate = { providerName: "acme", provider: { baseUrl: "https://example.invalid/v1", api: "openai-completions", apiKey: "!must-not-execute" }, model: { id: "unregistered-model", name: "Model A" } };
  ...
  const rejected = await throwsCode(() => handleModelsRequest("providers.test", invalid, ctx), "models_config_invalid");
  assert.ok(rejected.details.issues.some((issue) => issue.path.endsWith("baseUrl")));
  const invalidWeb = await post(invalid);
  assert.equal(invalidWeb.status, 200);
  assert.equal((await invalidWeb.json()).ok, false);
  const native = await handleModelsRequest("providers.test", candidate, ctx);
  const web = await (await post(candidate)).json();
  for (const result of [native, web]) {
    assert.equal(result.ok, true);
    assert.equal(typeof result.latencyMs, "number");
    assert.match(result.responseText, /valid configuration/);
    assert.doesNotMatch(result.responseText, /resolved/);
  }
