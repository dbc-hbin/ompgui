...
test("settings.update rejects malformed optional settings payloads", async () => {
  const restore = isolateAgentDir();
  try {
    const noOp = await handleSystemRequest("settings.update", {}, ctx);
    assert.equal(noOp.success, true);
    assert.equal(noOp.scope, "global");
    await throwsCode(
...
test("settings patches preserve siblings and reject out-of-range retries before persistence", async () => {
  ...
  try {
    ...
    assert.deepEqual(out.effectiveSettings, out.settings);
    assert.ok(Array.isArray(out.catalog) && out.catalog.length > 300);
    assert.equal(typeof out.secretStatus["auth.broker.token"], "boolean");
