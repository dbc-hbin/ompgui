...
const { attachRelayConnection, revokeRelayDeviceConnections } = await jiti.import("./connection.ts");
const { relayAttachments } = await jiti.import("./attachments.ts");
const { handleFilesRequest } = await jiti.import("./files-requests.ts");
...
test("older socket close and navigation preserve device staging until final disconnect or revoke", async (t) => {
  const deviceId = "concurrent-staging-device";
  const dir = mkdtempSync(join(tmpdir(), "relay-reconnect-"));
  allowFileRoot(dir);
  const connections = [];
  t.after(() => { for (const connection of connections) connection.onClose(); revokeRelayDeviceConnections(deviceId); rmSync(dir, { recursive: true, force: true }); });
  const connect = async () => {
    const socket = fakeSocket();
    const connection = attachRelayConnection(socket, {
      authenticate: () => ({ ok: true, serverId: "s_test", deviceId }),
      ...noopTimers(),
    });
    connections.push(connection);
    connection.onText(helloPairing());
    await flush();
    return { connection, socket };
  };
  const a = await connect();
  const b = await connect();
  const image = relayAttachments.begin(deviceId, "image/png", 1);
  const context = { deviceId, sessionId: null };
  const upload = await handleFilesRequest("uploadBegin", { dir, file: "pending", size: 0 }, context);
  a.connection.onText(JSON.stringify({ op: "session.close" }));
  await flush();
  a.connection.onClose();
  relayAttachments.chunk(deviceId, image.attachmentId, 0, "eA==");
  await handleFilesRequest("uploadComplete", { transferId: upload.transferId }, context);
  const pending = await handleFilesRequest("uploadBegin", { dir, file: "final", size: 0 }, context);
  b.connection.onClose();
  assert.throws(() => relayAttachments.complete(deviceId, image.attachmentId), { code: "invalid_attachment" });
  await assert.rejects(handleFilesRequest("uploadComplete", { transferId: pending.transferId }, context), { code: "transfer_not_found" });
  const c = await connect();
  const d = await connect();
  const revoked = relayAttachments.begin(deviceId, "image/png", 1);
  await handleFilesRequest("uploadBegin", { dir, file: "revoked", size: 0 }, context);
  revokeRelayDeviceConnections(deviceId);
  assert.equal(c.socket.getClosed().code, 1008);
  assert.equal(d.socket.getClosed().code, 1008);
  assert.throws(() => relayAttachments.chunk(deviceId, revoked.attachmentId, 0, "eA=="), { code: "invalid_attachment" });
  assert.deepEqual(readdirSync(dir), ["pending"]);
});
...
test("pairing hello then lists sessions and opens a snapshot", async () => {
  ...
  assert.deepEqual(
    { total: snapshot.total, offset: snapshot.offset, nextOffset: snapshot.nextOffset, hasMore: snapshot.hasMore },
    { total: 201, offset: 101, nextOffset: 201, hasMore: false },
  );
...
test("session.create lists projects and auto-opens the new session", async () => {
  ...
  const snapshot = socket.outgoing.find((frame) => frame.op === "session.snapshot");
  assert.equal(snapshot.id, "sess-new");
  assert.deepEqual(
    { total: snapshot.total, offset: snapshot.offset, nextOffset: snapshot.nextOffset, hasMore: snapshot.hasMore },
    { total: 0, offset: 0, nextOffset: 0, hasMore: false },
  );
});
...
test("session.export, session.leaf, git.diff, skills.toggle, plugins.action, mcp.delete, and mcp.upsert emit server frames", async () => {
  ...
  assert.deepEqual(
    { total: snapshot.total, offset: snapshot.offset, nextOffset: snapshot.nextOffset, hasMore: snapshot.hasMore },
    { total: 77, offset: 27, nextOffset: 77, hasMore: false },
  );
