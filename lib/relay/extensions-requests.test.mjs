...
test("agents.setDisabled and setOverride round-trip through native settings", async () => {
  ...
  try {
    ...
    await handleExtensionsRequest("agents.setOverride", { name: "scout", kind: "advisor", value: true }, ctx);
    assert.equal(readNativeSettings().settings.task.agentAdvisor.scout, "on");
