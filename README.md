...
> **Android APK (Android 12+)** — Use the Kotlin companion app to connect to a remote ompgui server, with a read-only offline snapshot of the latest session. [Download ompgui Remote v0.7.4](https://github.com/dbc-hbin/ompgui/releases/download/v0.7.4/ompgui-remote-v0.7.4.apk) · [Release notes](https://github.com/dbc-hbin/ompgui/releases/tag/v0.7.4)
...
>
> New in v0.7.0: Settings redesigned with tabbed categories and always-visible search (web-like parity), all Strings migrated to string resources (English/Korean/Japanese/Chinese complete), Usage sheet redesigned with capacity chips and color-coded progress bars, and tighter session/message spacing.
>
...
- **Queued messages**: web and current Android clients share a server-owned, in-memory queue. On Android, open **Session controls → Queue** explicitly; typing does not add an automatic Execution/Queue bar. Recall restores text and images, delete discards an eligible item, and promotion changes a follow-up to steering subject to safe dispatch. Sending items cannot be edited. Snapshots expose image metadata, not private image bytes; count, byte, and memory limits apply. Relay recalls over 15 MiB are refused before mutation and direct you to the web client. A server-process restart does not preserve pending items; older deployed clients retain compatible prompt entry points.
- **Session search**: choose metadata or indexed body search, with inclusive start/exclusive end dates and read-only match context. The private Node/SQLite index covers visible conversation text, not credentials, hidden reasoning, or opaque payloads; search does not modify history.
- **Provider checks**: configuration validation is not a network test. A real connectivity test requires explicit approval of possible cost, uses a fixed prompt with a 32-token output limit, and sends at most one provider request. It supports OpenAI Chat Completions, OpenAI Responses, and Anthropic Messages API shapes with literal candidate credentials only—not stored OAuth credentials.
